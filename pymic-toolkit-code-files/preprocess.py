...
def preprocess_promis12(data_dir):
        ...
        imagePathInput = os.path.join(data_dir, 'data/')
        labelPathInput = os.path.join(data_dir, 'label/')
         
        labelfiles=os.listdir(labelPathInput)
        file_names = os.listdir(imagePathInput)
        print(file_names)
        
        for file_name in file_names:
                print(file_name)
                
                img_name_full = "{0:}{1:}".format(imagePathInput, file_name)
                lab_name = file_name.replace(".mhd", "_segmentation.mhd")
                lab_name_full = "{0:}{1:}".format(labelPathInput, lab_name)
                print(img_name_full)
                out_name = file_name
...
if __name__ == "__main__":
    data_dir = "/content/drive/MyDrive/Colab Notebooks/HeadNeck_GTV/dataroot/origindata/"
